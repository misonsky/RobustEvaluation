...
class FinetuningConfig(object):
  ...
  def __init__(self,args,**kwargs):
    ...
    self.do_train = args.do_train  # train a model
    self.do_eval = args.do_eval  # evaluate the model
    self.keep_all_models = True  # if False, only keep the last trial's ckpt

    # model
    self.model_size = "base"  # one of "small", "base", or "large"
    self.task_names = [args.task_names]  # which tasks to learn
    ...
    self.num_train_epochs = args.num_train_epochs  # passes over the dataset during training
    self.warmup_proportion = 0.1  # how much of training to warm up the LR for
    self.save_checkpoints_steps = args.save_checkpoints_steps
    ...
    pretrained_model_dir = os.path.join(self.data_dir, "models", args.model_name)
    self.raw_data_dir = os.path.join(self.data_dir, "finetuning_data", "{:}").format
    ...
    self.model_dir = os.path.join(pretrained_model_dir, "finetuning_models",task_names_str + "_model")
    ...
    #alter the config for conversation
    self.corpus=args.corpus #douban,mutual,ecomm
    self.data_dir=args.data_dir
    self.model_name = args.model_name
    self.tfrecords_dir=args.tfrecords_dir
    pretrained_model_dir=args.model_path
    self.vocab_file = os.path.join(pretrained_model_dir, "vocab.txt")
    self.init_checkpoint = None if self.debug else pretrained_model_dir
    self.model_dir=os.path.join(args.output_path,self.corpus,self.model_name)
    self.results_txt = os.path.join(self.model_dir,"results.txt")
    self.preprocessed_data_dir = os.path.join(self.tfrecords_dir, self.corpus,self.model_name)
    self.test_predictions = os.path.join(
        pretrained_model_dir, "test_predictions",
        "{:}_{:}_{:}_predictions.pkl").format
