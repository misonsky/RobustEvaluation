...
from BERTModel.BERT import modeling
from BERTModel.BERT import optimization
from BERTModel.BERT import tokenization
...
def main(_):
  ...
  if not FLAGS.do_train and not FLAGS.do_eval and not FLAGS.do_predict and not FLAGS.do_prepare:
    raise ValueError(
        "At least one of `do_train`, `do_eval` or `do_predict' or `do_prepare` must be True.")
  ...
  if FLAGS.do_train:
    ...
    tf.logging.info("  Num steps = %d", meta_json["num_train_steps"])
