  ...
  def _load_glue(self, lines, split, text_a_loc, text_b_loc, label_loc,skip_first_line=False, eid_offset=0, swap=False):
    ...
    for (i, line) in enumerate(lines):
      try:
        ...
        text_a = tokenization.convert_to_unicode(history)
        text_b = tokenization.convert_to_unicode(response)
...
class CONV(ClassificationTask):
    def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
        super(CONV, self).__init__(config, "conv", tokenizer,["0", "1"])
    ...
    def read_txt(self,input_file):
        ...
        with open(input_file, "r",encoding="utf-8") as f:
            for line in f:
                ...
                line=line.rstrip()
                yield line
    def get_examples(self, data_dir,corpus,split):
        fileName=None
        if split=="train":
            fileName="train.txt"
        elif split=="dev":
        ...
        return self._create_examples(self.read_txt(os.path.join(data_dir, corpus, fileName)), split)
    def _create_examples(self, lines, split):
        examples = []
        examples += self._load_glue(lines, split, -3, -2, -1, skip_first_line=False)
        return examples
