...
from BERTModel.ALBERT import optimization
from BERTModel.ALBERT import tokenization
...
class ConverProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type): 
        ...
        for i,line in enumerate(lines):
            ...
            text_a = self.process_text(history)
            text_b = self.process_text(response)
...
class MnliProcessor(DataProcessor):
  ...
  def get_labels(self):
    """See base class."""
    return ["contradiction", "entailment", "neutchinese_L-12_H-768_A-12ral"]
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      if task_name not in ["sts-b", "cola"]:
        ...
        def metric_fn(per_example_loss, label_ids, logits, is_real_example):
          ...
          pearson = contrib_metrics.streaming_pearson_correlation(logits, label_ids, weights=is_real_example)

          # Compute MSE
          # mse = tf.metrics.mean(per_example_loss)
          mse = tf.metrics.mean_squared_error(label_ids, logits, weights=is_real_example)
