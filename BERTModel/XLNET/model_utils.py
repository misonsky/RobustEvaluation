...
def get_train_op(FLAGS, total_loss, grads_and_vars=None):
  ...
  if meta_json["num_warmup_steps"] > 0:
  ...
  if FLAGS.decay_method == "poly":
    decay_lr = tf.train.polynomial_decay(
        FLAGS.learning_rate,
        global_step=global_step - meta_json["num_warmup_steps"],
        decay_steps=meta_json["num_train_steps"] - meta_json["num_warmup_steps"],
  ...
  learning_rate = tf.where(global_step < meta_json["num_warmup_steps"],warmup_lr, decay_lr)
